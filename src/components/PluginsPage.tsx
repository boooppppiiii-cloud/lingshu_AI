...
const PLUGIN_FIELDS: Record<string, { key: string; label: string; placeholder: string; secret?: boolean }[]> = {
  ...
  whatsapp_business: [
    ...
    { key: 'accessToken', label: 'Access Token (永久口令)', placeholder: 'EAABxxxxx...', secret: true },
    { key: 'verifyToken', label: 'Verify Token（Webhook 验证字符串）', placeholder: '自定义字符串，如 my_verify_token_2024' },
  ],
  google_translate: [
...
interface WhatsAppOnboardingInfo {
  canUseEmbeddedSignup: boolean;
  appId: string;
  configId: string;
  graphVersion: string;
  webhookUrl: string;
...
interface WhatsAppEmbeddedSignupResult {
  code: string;
  phoneNumberId: string;
  wabaId: string;
}
...
function readWhatsAppSignupMessage(event: MessageEvent) {
  ...
  const typed = payload as { type?: string; data?: Record<string, unknown> };
...
async function startWhatsAppEmbeddedSignup(info: WhatsAppOnboardingInfo): Promise<WhatsAppEmbeddedSignupResult> {
  await loadFacebookSdk(info.appId, info.graphVersion);

  return new Promise((resolve, reject) => {
    if (!window.FB) {
      reject(new Error('Meta SDK 未就绪'));
      return;
    }

    let latestSignupData = { phoneNumberId: '', wabaId: '' };
    let settled = false;
    let timeout = 0;

    function handleMessage(event: MessageEvent) {
      const signupData = readWhatsAppSignupMessage(event);
      if (!signupData) return;
      latestSignupData = {
        phoneNumberId: signupData.phoneNumberId || latestSignupData.phoneNumberId,
        wabaId: signupData.wabaId || latestSignupData.wabaId,
      };
    }
    ...
    timeout = window.setTimeout(() => {
      cleanup();
      reject(new Error('Meta 授权超时，请重新打开连接窗口'));
    }, 5 * 60 * 1000);
    ...
    window.FB.login((response) => {
      ...
      cleanup();
      if (!code) {
        reject(new Error('未完成 Meta 授权'));
        return;
      }
      resolve({ code, ...latestSignupData });
    }, {
...
function WhatsAppConnectionPanel({
  ...
  const [info, setInfo] = useState<WhatsAppOnboardingInfo | null>(null);
  ...
  async function refreshOnboarding() {
    setLoadingInfo(true);
    try {
      const r = await fetch('/api/overseas/plugins/whatsapp_business/onboarding');
      const data = await r.json() as WhatsAppOnboardingInfo | { error?: string };
      if (!r.ok) throw new Error('error' in data ? data.error : '无法读取 WhatsApp 连接状态');
      const next = data as WhatsAppOnboardingInfo;
      setInfo(next);
      setConfigValues(prev => ({
        ...prev,
        verifyToken: prev.verifyToken || next.verifyToken,
        graphVersion: prev.graphVersion || next.graphVersion,
      }));
    } catch (err) {
      setNotice({ ok: false, msg: err instanceof Error ? err.message : '无法读取 WhatsApp 连接状态' });
    } finally {
      setLoadingInfo(false);
    }
  }

  useEffect(() => { void refreshOnboarding(); }, []);

  async function connect() {
    if (!info || !info.canUseEmbeddedSignup) return;
    setConnecting(true);
    setNotice(null);
    try {
      const signup = await startWhatsAppEmbeddedSignup(info);
      const r = await fetch('/api/overseas/plugins/whatsapp_business/embedded-signup/complete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(signup),
      });
      const data = await r.json() as { ok: boolean; message?: string; error?: string; webhookSubscribeError?: string };
      if (!r.ok || !data.ok) throw new Error(data.error ?? 'WhatsApp 授权失败');
      setNotice({
        ok: true,
        msg: data.webhookSubscribeError
          ? `${data.message ?? 'WhatsApp 已连接'}，但消息订阅需要管理员复核`
          : (data.message ?? 'WhatsApp 已连接'),
      });
      await refreshOnboarding();
      await onConnected();
    } catch (err) {
      setNotice({ ok: false, msg: err instanceof Error ? err.message : 'WhatsApp 授权失败' });
    } finally {
      setConnecting(false);
    }
  }
  ...
  const connectedTitle = status?.displayName || 'WhatsApp Business';
  const connectedSubtitle = status?.displayPhoneNumber || status?.phoneNumberId || '已完成授权';
  const advancedFields = PLUGIN_FIELDS[plugin.pluginKey] ?? [];

  return (
    <>
      <div className="space-y-4">
        <div className={`rounded-xl p-4 ${status?.connected ? 'bg-green-50 border border-green-100' : 'bg-gray-50 border border-gray-100'}`}>
          <div className="flex items-start gap-3">
            <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${status?.connected ? 'bg-green-100 text-green-700' : 'bg-white text-gray-500 border border-gray-200'}`}>
              {status?.connected ? <ShieldCheck size={18} /> : <MessageSquare size={18} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-gray-900">
                  {status?.connected ? '已连接 WhatsApp Business' : '连接 WhatsApp Business'}
                </p>
                {loadingInfo && <span className="text-xs text-gray-400">检查中...</span>}
              </div>
              <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                {status?.connected
                  ? `${connectedTitle} · ${connectedSubtitle}`
                  : '客户登录 Meta、选择或创建 WhatsApp 商业号码后，系统会自动保存授权。'}
              </p>
              {status?.connected && status.webhookSubscribeError && (
                <p className="mt-2 text-xs text-amber-600">{status.webhookSubscribeError}</p>
              )}
            </div>
          </div>
        </div>
        ...
        {notice && (
          <div className={`text-xs px-3 py-2 rounded-lg flex items-start gap-1.5 ${notice.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
            {notice.ok ? <CheckCircle size={12} className="mt-0.5" /> : <AlertCircle size={12} className="mt-0.5" />}
            <span>{notice.msg}</span>
          </div>
        )}

        {testResult && (
          <div className={`text-xs px-3 py-2 rounded-lg flex items-start gap-1.5 ${testResult.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
            {testResult.ok ? <CheckCircle size={12} className="mt-0.5" /> : <AlertCircle size={12} className="mt-0.5" />}
            <span>{testResult.msg}</span>
          </div>
        )}

        {info && info.missingSetup.length > 0 && (
          <div className="rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs text-amber-700">
            <p className="font-medium">{info.canUseEmbeddedSignup ? '管理员提醒' : '管理员待完成 Meta 应用配置'}</p>
            <p className="mt-1 leading-relaxed">
              {info.canUseEmbeddedSignup
                ? '当前可以授权，生产环境还需要确认公网域名用于接收 WhatsApp 消息。'
                : '配好后客户只需要点击连接，不需要填写 Phone Number ID、Access Token 或 Verify Token。'}
            </p>
            <p className="mt-2 font-mono text-[11px] break-all">{info.missingSetup.join(' / ')}</p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={!info?.canUseEmbeddedSignup || connecting || loadingInfo}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm text-white font-medium disabled:opacity-50 transition-colors"
            style={{ background: '#16a34a' }}
          >
            <ExternalLink size={15} /> {connecting ? '连接中...' : status?.connected ? '重新授权' : '连接 WhatsApp Business'}
          </button>
          {status?.connected && (
            <button
              onClick={onTest}
              disabled={testing}
              className="px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600 disabled:opacity-50 hover:bg-gray-50"
            >
              {testing ? '测试中...' : '测试'}
            </button>
          )}
        ...
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2 border border-gray-200 rounded-xl text-xs text-gray-600 hover:bg-gray-50"
        >
          <span className="flex items-center gap-1.5"><Lock size={12} /> 高级实施配置</span>
        ...
        {advancedOpen && (
          <div className="space-y-3 rounded-xl border border-gray-200 p-3">
            ...
            <div className="p-3 bg-blue-50 rounded-xl text-xs text-blue-700 space-y-1.5">
              <p className="font-semibold">Webhook 信息</p>
              <code className="block font-mono bg-blue-100 px-2 py-1.5 rounded-lg break-all select-all">
                {info?.webhookUrl ?? `${window.location.origin}/api/overseas/plugins/whatsapp_business/webhook`}
              </code>
              <p className="text-blue-500">Verify Token 使用系统生成的字符串，订阅字段选择 <strong>messages</strong>。</p>
            </div>
            <button
      ...
      <div className="flex gap-3 mt-5">
        <button onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600">关闭</button>
      </div>
    </>
  );
}
              ...
              <div key={cat} className="mb-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {catPlugins.map(plugin => {
                    ...
                    const isWhatsApp = plugin.pluginKey === 'whatsapp_business';
                    return (
                            ...
                            {!plugin.installed ? (
                              ...
                              <>
                                {fields.length > 0 && (
                                  ...
                                  >
                                    {isWhatsApp ? <MessageSquare size={12} /> : <Settings size={12} />}
                                    {isWhatsApp ? (plugin.status === 'installed' ? '管理连接' : '连接') : '配置'}
                                  </button>
                                )}
                                {(!isWhatsApp || plugin.status === 'installed') && (
                                  <button
        ...
        {configTarget && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-2xl w-[520px] max-w-[calc(100vw-32px)] p-6"
            ...
            >
              ...
              {configTarget.pluginKey === 'whatsapp_business' ? (
                <WhatsAppConnectionPanel
                  plugin={configTarget}
                  configValues={configValues}
                  setConfigValues={setConfigValues}
                  onClose={() => setConfigTarget(null)}
                  onSaveManual={() => saveConfig(configTarget)}
                  onConnected={fetchPlugins}
                  onTest={() => void testPlugin(configTarget.pluginKey)}
                  testing={testing === configTarget.pluginKey}
                  testResult={testResult[configTarget.pluginKey]}
                />
              ) : (
                <>
                  <div className="space-y-4">
