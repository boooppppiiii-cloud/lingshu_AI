...
import { AlertTriangle, ExternalLink, Loader2, RefreshCcw, Upload } from 'lucide-react';
...
export default function AdminContentOpsAlerts() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const resp = await fetch('/api/overseas/admin/video-alerts?limit=20', { headers: authHeader() });
  ...
  const uploadManualVideo = async (alert: VideoAdminAlert) => {
    ...
    try {
      ...
      setMessage('已上传并进入 Gemini 分析队列，分析成功后会自动出现在灵感大屏。');
      await load();
    } catch (err) {
  ...
  return (
    <section className="rounded-3xl border border-border bg-surface p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-text-muted">处理视频分析失败、人工补传和重新入库。</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-text-secondary">
            {loading ? '读取中' : `${alerts.length} 条待看`}
          </span>
      ...
      {alerts.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-border bg-white px-3 py-4 text-xs text-text-muted">暂无需要人工处理的视频分析失败。</p>
      ) : (
