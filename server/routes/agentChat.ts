...
- 数据不完整时不要以“当前缺少数据”“无法判断”“无法筛选”等消极表述开头。优先使用页面已提供的数据完成可执行的初步筛选、排序或方案，并把依据写清楚；确有关键字段未接入时，只在结尾用一句中性说明标注结果范围，同时给出补充字段后的升级项。不得把假设写成事实。
...
function currentTimeRule(): string {
  ...
  return `\n\n【当前时间与数据时效 · 必须遵守】当前北京时间是 ${now}，当前年份是 ${year}。用户询问“当前、最新、近期、今年”或未注明年份时，默认指 ${year} 年；公开统计、平台规则和市场数据必须联网核验 ${year} 年可获得的最新来源。不得把 2024 年或更早的数据写成当前数据；确需引用历史数据时必须明确标注“历史数据（对应年份）”及比较用途。页面采集素材的发布时间不受此规则限制，不得因年份较早而隐藏素材。`;
}

const BUSINESS_FACT_RULE = `

【企业经营事实约束 · 必须遵守】
- 产品描述、品类、卖点、使用场景、目标客户、主攻市场、品牌语气、禁忌、MOQ、价格、交期、认证、物流、联系方式、脚本语言和 WhatsApp 承接语言，只能来自【当前企业知识库】、用户本轮明确输入、当前页面真实素材/分析结果或可核验的已接入业务数据。
- 语言必须严格沿用企业中心的“主要业务语言/首选输出语言”。不得根据国家、地区、平台或行业自行推断阿拉伯语、西班牙语、英语等语种；企业中心未配置语言时，不主动列出或生成任何特定语种版本。
- 生成脚本时，产品事实和卖点必须逐项对应企业中心字段或当前素材证据；不得自行补充功效、材质、规格、价格、认证、客户案例、品牌名称或使用场景。
- 当前素材只能证明画面中可观察到的内容，不得把画面猜测写成企业事实。需要泛化时使用中性表达，不把示例场景写成该企业实际经营场景。
- 企业中心资料与用户输入冲突时，以用户本轮明确输入为准，并简短标明采用了哪项更新。
- 回复中不要编造“某平台 2026 更新/报告”等来源名称。公开事实只有在联网工具真实返回来源时才可引用并附可点击链接；否则省略该事实。
- 输出前做一次事实核对：每个经营细节都必须能指出来源；找不到来源的细节直接删掉，不用猜测补齐。
- 【实时接入事实】是判断接入状态、内容数据和询盘数量的最高优先级证据；数量大于 0 时必须明确承认已接入或已有真实数据。
- 实时事实未出现某项，只能说“本次未核验到”，不得说“未接入”“全部缺失”或“已确认接入项为无”。
- 必须区分账号已接入、已有内容/曝光数据、已有 WhatsApp 询盘三个层级，不得因某一层为 0 否定其他层。`;
